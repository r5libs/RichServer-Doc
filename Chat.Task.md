...
## 功能: getAllOfficialTaskIds 取得目前所有官方任務的 ID
...
response:
  ...
  case error == 0:
    ...
    string[] meetConditions; 任務達成條件列表
      ...
      揪一起:
        201: [0]: 目標GPS位置, [1]: 報到時間(玩家到達指定地點才可以報到. 所有人都報到可忽略開始時間/報到人數須>=2/有報到的一定要確認開始), [2]: 開始時間(報到後, 玩家才可以開始任務(下一階段)) [3]: 結束時間(任務沒完成就算失敗), [4]: 聊天句數
