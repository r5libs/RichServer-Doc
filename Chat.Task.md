...
response:
  ...
  case error == 0:
    ...
    string[] meetConditions; 任務達成條件列表
      ...
      揪一起:
        201: [0]: 目標GPS位置, [1]: 報到時間(玩家到達指定地點才可以報到. 所有人都報到可略開始時間/報到人數須>=2/有報到的一定要確認開始), [2]: 開始時間(報到後, 玩家才可以開始任務(下一階段)) [3]: 結束時間(任務沒完成就算失敗)
    ...
    string[] dataList; 任務資料. 透過updateTask更新
      揪運動:
        ...
        102: [0]: 所有玩家總里程
      揪一起:
        201: [0]: 0: 等候報到, 1: 等候開始(下一階段), 2: 進行聊天100句, [1]: 所有玩家總聊天句數
